...
import { ScrollView, Modal, View, TouchableOpacity, Text, Image } from 'react-native';
...
import {
  ...
  SortModalContainer, SortModalContent, SortModalTitle, SortOptionButton, SortOptionText,
  SortCancelButton, SortCancelText, CancelTaskButton, CancelTaskButtonText,
  ImageContainer, EmptyMessage
} from './styles';
    ...
    <Container>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: RFValue(15), marginTop: RFValue(10), marginBottom: RFValue(5) }}>
        <TouchableOpacity onPress={() => setSortModalVisible(true)}>
          <Text style={{ fontSize: RFValue(12), fontWeight: 'bold', color: '#618DDB' }}>
            Order by: {sortOptions.find(opt => opt.value === currentSortOption)?.label}
          </Text>
        </TouchableOpacity>
      ...
      <Content>
        <ScrollView contentContainerStyle={{ padding: RFValue(20), gap: RFValue(10), flexGrow: 1 }}>
          {tasks.length === 0 ? (
            <ImageContainer>
              <Image source={require('../../assets/EmptyList.png')} style={{ flex: 1, aspectRatio: 1 }} resizeMode='contain' />
              <EmptyMessage>You have no tasks here...</EmptyMessage>
            </ImageContainer>
          ) : (
            tasks.map((item) => (
